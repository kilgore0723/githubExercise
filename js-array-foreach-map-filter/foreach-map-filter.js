...
function showFirstAndLast(arr) {
  ...
  arr.forEach(function (value) {
    for (let i = 0; i < value.length; i++) {
      return showFirstLast.push(value[i] + value[value.length - 1]);
    }
  });
...
//   strings.forEach(function (keyValues) {
//     for (let char of keyValues) {
...
function vowelCount(str) {
  ...
  let object = {};
  let strings = str.toLowerCase().split("");
  strings.forEach(function (char) {
    if (vowels.includes(char)) {
      object[char] = object[char] ? object[char] + 1 : 1;
    }
  });
  return object;
}
...
function removeVowels(str) {
  let vowels = "aeiou";
  let strings = str.toLowerCase().split("");
  let cons = strings.filter(function (value) {
